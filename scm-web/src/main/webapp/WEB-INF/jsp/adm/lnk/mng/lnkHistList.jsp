...
<script>
	...
	const eventHandler = {
		handlers: {
			...
			, lnkInst: {  handler: 'changeLnkInst', eventType: 'change' }
		}
		, init: function(){
			this.bind();
		}
		...
		, clickResetButton: function(e){
			document.getElementById('lnkInst').value = '';
			document.getElementById('lnkNm').value = '';
		...
		, changeLnkInst: function(e){
			//1. 선택된 기관값 가져오기
			const selectedInstVal = document.getElementById('lnkInst').value; 
			//2. 기관값이 '' 이면 연계명도 비워두기
				const lnkList = document.getElementById('lnkNm');
			if(!selectedInstVal){
				lnkList.innerHTML = "";
				const allOption = document.createElement('option');
			      allOption.value = "";
			      allOption.textContent = "연계 전체";
			      lnkList.appendChild(allOption);
			}
			else{
				lnkList.innerHTML = "";
				//1. 먼저 전체 추가
				const allOption = document.createElement('option');
				 allOption.value = "";
			      allOption.textContent = "연계기관 전체";
			      lnkList.appendChild(allOption);
			     
			      
			      instObject.forEach((instData, no ) => {
			    	  if(selectedInstVal == instData.inst){
			    		  const lnkOption = document.createElement('option');
							 lnkOption.value = instData.lnk;
						      lnkOption.textContent = instData.lnk;
						      lnkList.appendChild(lnkOption);
			    	  }
			      })
			      
			}
			
			//3. 기관값을 저장되어있는 object랑 비교해서 해당 row의 inst값이랑 동일할 경우만 연계명에 추가하기
	...
	const search = (pageNo = 1) => {
		...
		const operList = document.getElementById('lnkInst').value; 
		const linkName = document.getElementById('lnkNm').value; 
		...
		const url = '/adm/lnk/mng/selectLnkHistList.json'
	...
	const renderTable = (list) => {
		...
		if(!list){
			lnkApiList.innerHTML = '<tr><td colspan="6">조회된 데이터가 없습니다.</td></tr>';
			return;
		}
		columns = ['linkInstNm', 'btchExpln', 'btchExcnStngVl', 'btchExcnDt', 'scsYn','mdfcnNocs']
		...
		list.forEach((data) => {
			...
			for(const col of columns){
				...
				if(data[col] === 'Y'){
					...
					td.innerHTML = "<span class='label label-success'>진행중</span>";
				}
				else{
	...
	const setOperList = (data,currVal) => {
	  ...
	  const operList = document.getElementById('lnkInst');
	  operList.innerHTML = "";
		  // Add the "[전체]" option
      const allOption = document.createElement('option');
      allOption.value = "";
      ...
      data.forEach(item => {
          ...
          option.value = item.linkInstNm;
          option.textContent = item.linkInstNm;
          operList.appendChild(option);
          
          // 하위 select를 위해 기관, 연계 데이터 저장해놓기
          
          const pushData = { inst: item.linkInstNm, lnk: item.btchExpln};
          instObject.push(pushData);
          
      });
	      operList.value = currVal;
...
<!-- ---------------------------------------------------------------------------------------------------------------- -->
<!-- 페이지 타이틀 시작 -->
						<h1 class="page-title-1depth"><span>기관별 데이터 연계 현황</span></h1>
						<!-- 페이지 타이틀 끝 -->
										...
										<div class="form-inline row">
											<div class="input-group  col-lg-4 col-md-4 col-sm-4 col-xs-4">
												<span class="input-group-label">
													<label class="input-label-display" for="lnkInst">검색 조건</label>
												</span>
												<select id="lnkInst" class="form-control">
													<option value="">연계기관 선택</option>
													<option>선택1</option>
													<option>선택2</option>
												</select>
											</div>
											<div class="input-group col-lg-4 col-md-4 col-sm-4 col-xs-4">
												<span class="input-group-label">
													<label class="input-label-none" for="lnkNm">연계 선택</label>
												</span>
												<select id="lnkNm" class="form-control">
													<option value="">연계 전체</option>
												</select>
											</div>
											<div class="input-group col-lg-4 col-md-4 col-sm-4 col-xs-4">
												<span class="input-group-label">
									...
									<table class="table table-bordered table-striped table-hover" >
										...
										<thead>
											<tr>
												...
												<th scope="col"> 연계명 </th>
												<th scope="col"> 연계주기 </th>
												<th scope="col"> 최근실행이력 </th>
												<th scope="col"> 성공여부 </th>
												<th scope="col"> 업데이트 건수
												</th>
											</tr>
										</thead>
										<tbody id="lnkApiList">
