...
<script>
	...
	document.addEventListener('DOMContentLoaded', () => {
		...
		const addFileButton = document.querySelector('.input-group-btn.input-group-last');
		if(addFileButton) addFileButton.remove();
	...
	const eventHandler = {
		handlers: {
			...
			,createPopButton: { handler: 'createPopClick', eventType: 'click' }
		...
		, previewPopClick: function(e){
			if(!submitForm.validNull() || !submitForm.validDateFormat()){
				return;
			}
	...
	const submitForm = {
		...
		, create: function(){
			...
			const createUrl = '/adm/bbs/pop/createPop.json';
		...
		, validNull: function(){
			...
			if(!params.admPopGroupSn.fileGroupSn){
				alert('이미지 등록은 필수입니다.');	
				return false;
			}
	...
	<!-- 페이지 타이틀 시작 -->
	<h1 class="page-title-1depth"><span>공지 팝업 관리 등록 - 
		<c:choose>
		...
		<div class="contents-row">
			...
			<div id="section1" class="table-scrollable">
				<table class="table table-bordered">
					<caption>공지 팝업 등록을 위한 상세 입력 테이블</caption>
					<colgroup>
					...
					<tbody>
						<tr>
							<th class="td-head" scope="row">제목 <span class="textR">*</span></td>
							<td>
								<label class="input-label-none" for="popupTtl">제목 입력</label>
								<input type="text" id="popupTtl" class="form-control required" placeholder="팝업의 제목을 입력하세요." style="width:100%;">
							</td>
						</tr>
						<tr>
							<th class="td-head" scope="row">팝업 이미지 <span class="textR">*</span></th>
							<td>
                        		<div id="admPopGroupSn" class="required"></div>
							</td>
						</tr>
						<tr>
							...
							<td>
								<div class="form-inline">
									...
									<div class="input-group date date-picker" data-date="" data-date-format="yyyy-mm-dd" data-date-viewmode="years">
										<input type="text" id="popupBgngDt" name="popupBgngDt" value="" title="시작일" maxlength="10" class="form-control required">
									...
									<div class="input-group date date-picker" data-date="" data-date-format="yyyy-mm-dd" data-date-viewmode="years">
										<input type="text" id="popupEndDt" name="popupEndDt" value="" title="종료일" maxlength="10" class="form-control required">
										<span class="input-group-btn">
											<button class="btn btn-default" type="button">
												<i class="fa fa-calendar"></i>
											</button>
						...
						<tr>
							...
							<td>
								...
								<input type="text" id="acsUrlAddr" class="form-control" placeholder="팝업 이미지 클릭 시 이동할 페이지 URL을 입력하세요." style="width:100%;">
							</td>
						</tr>
						<tr>
			...
			<div class="table-bottom-control">
				...
				<button type="button" class="btn btn-primary" id="createPopButton">등록</button>
			</div>
	...
	<!-- -------------------------------------------------------------------------------------------------- -->
	<!-- 모달팝업 시작 -->
	<div class="modal fade" id="preview" tabindex="-1" role="preview" style="display: none; padding-right: 17px;">
				...
				<div class="modal-body">
					...
					<div class="contents-row" id="previewCn">
						<div class="" id="pre" style="text-align: center;">
							<img alt="미리보기 이미지" id="previewImg" src="" style="width: 100%;"/>
						</div>
					</div>
					<!-- 내용 끝 -->
				</div>
