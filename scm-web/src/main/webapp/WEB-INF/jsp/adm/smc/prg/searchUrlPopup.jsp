    ...
    $(function() {
        ${POPUP_ID} = {
            ...
            , search : function (pageNo) {
                const pageSize = 10;
                ${POPUP_ID}.condition["searchUrlId"] = $("#${POPUP_ID}").find("input[name=searchUrlId]").val();
                ...
                const url = '/adm/smc/prg/selectUrlList.json';

                console.log(${POPUP_ID}.condition);

                sendJson(url, ${POPUP_ID}.condition, function (data){
                    console.log(JSON.stringify(data));
                    /* 총건수 */
                    ${POPUP_ID}.$("#popTotCnt").html(data.pagingVO.totalCount);
            ...
            , renderTable : function (list) {
                ...
                if (!list || list.length === 0) {
                    ...
                    list.forEach(function (item, index) {
                        tableBody +=
                            "<tr>" +
                            "<td>" + (item.urlAddr || 'N/A') + "</td>" +
                            "<td>" + (item.prgrmNm || 'N/A') + "</td>" +
                            "<td><button type='button' class='btn btn-default' onclick='${POPUP_ID}.selectProgram(\"" + item.urlAddr + "\", \"" + item.prgrmNm + "\", \"" + item.rprsPrgrmYn + "\", \"" + item.mdfcnPrgrmYn + "\", \"" + item.prgrmClsfCd + "\")'>선택</button></td>" +
                            "<input type='hidden' name='hiddenValue' value='" + (item.rprsPrgrmYn || '') + "'>" +
                            "<input type='hidden' name='hiddenValue' value='" + (item.mdfcnPrgrmYn || '') + "'>" +
                            "<input type='hidden' name='hiddenValue' value='" + (item.prgrmClsfCd || '') + "'>" +
                            "</tr>";
                    });
                }
                // urlList tbody에 테이블 데이터 삽입
                ${POPUP_ID}.$('#urlList').html(tableBody);

            }
            , selectProgram : function (urlAddr, prgrmNm, rprsPrgrmYn, mdfcnPrgrmYn, prgrmClsfCd ) {

                let params 	= {};
                params.urlAddr = urlAddr;
                params.prgrmNm = prgrmNm;
                params.rprsPrgrmYn = rprsPrgrmYn;
                params.mdfcnPrgrmYn = mdfcnPrgrmYn;
                params.prgrmClsfCd = prgrmClsfCd;
                szms.popup.close("${POPUP_ID}", true, params);
            }
            , reset : function () {
                $("#${POPUP_ID}").find("input[name=searchUrlId]").val('');
...
<div class="modal-dialog modal-lg">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title">URL 검색</h4>
        </div>
                    ...
                    <div class="form-inline row">
                        <div class="input-group col-lg-5 col-md-5 col-sm-5 col-xs-12">
                            <span class="input-group-label">
                                <label class="input-label-display" for="searchUrlId">URL</label>
                            </span>
                            <input type="text" id="searchUrlId" name="searchUrlId" class="form-control" placeholder="URL을 입력하세요.">
                        </div>
