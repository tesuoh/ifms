...
<script>
	...
	const eventHandler = {
		...
		, clickMoveListButton: function(e){
			const params = {
					cdGroupId: document.getElementById('orgnlCdGroupId').value
					, cdId: document.getElementById('orgnlCdId').value
			}
			
			console.log(params);
			
			if(confirm('취소하시겠습니까? 변경사항이 저장되지 않습니다.')){
				sendForm('/adm/cdm/ccm/cmnCdMngDetail.do', savedParams);			
			}
		}
		...
		, clickSearchCdClsfButton: function(e){
			e.preventDefault();
			console.log('clickSearchCdClsfButton');
	...
	const searchCdClsfList = async (pageNo = 1) => {
		...
		await sendJson(url, params, (data) => {
			console.log('search cmnCdClsfList popup > ', data);
	...
	const submitForm = {
		...
		, params: function(){
			let result = {
				...
				, orgnlCdId: document.getElementById('orgnlCdId').value
			};
			...
			result.detailCdId = document.getElementById('cdId').value;
		...
		, update: function(){
			...
			const updateUrl = '/adm/cdm/ccm/updateCmnCdMng.json';
					
			sendJson(updateUrl, params, (res) => {
				if(res.success){
					sendForm('/adm/cdm/ccm/cmnCdMngDetail.do', params);
				}
			}, 
	...
	<!-- 페이지 타이틀 시작 -->
	<h1 class="page-title-1depth"><span>공통코드 수정</span></h1>
	<!-- 페이지 타이틀 끝 -->
					...
					<tbody>
						<tr>
							...
							<td>
								<div class="form-inline row">
									<div class="input-group col-lg-12 col-md-12 col-xs-12 col-sm-12">
										...
										<input type="text" id="cdGroupId" class="form-control" placeholder="" value="${detail.cdGroupId }" disabled>
							...
							<td>
								...
								<input type="text" id="cdGroupNm" class="form-control" placeholder=""  value="${detail.cdGroupNm }" disabled style="width: 100%;">
							</td>
						</tr>
						<tr>
							<td class="td-head" scope="row">코드ID <span class="textR">*</span></td>
							<td>
								<input type="hidden" id="orgnlCdId" value="${detail.cdId }"/>
								<label class="input-label-none" for="cdId">코드ID 입력</label>
								<input type="text" id="cdId" class="form-control" placeholder="" value="${detail.cdId }" style="width: 100%;">
							</td>
							<td class="td-head" scope="row">코드명 <span class="textR">*</span></td>
							<td>
								<label class="input-label-none" for="cdNm">코드명 입력</label>
								<input type="text" id="cdNm" class="form-control" placeholder="" value="${detail.cdNm }" style="width: 100%;">
							</td>
						</tr>
						<tr>
							<td class="td-head" scope="row">정렬순서 <span class="textR">*</span></td>
							<td>
								<div class="form-inline row">
									<div class="input-group col-lg-12 col-md-12 col-xs-12 col-sm-12">
										<label class="input-label-none" for="sortSeq">정렬순서</label>
										<input type="text" id="sortSeq" class="form-control" placeholder="" value="${detail.sortSeq }">
										<div class="input-group-btn input-group-last">
							...
							<td>
								...
								<select id="useYn" class="bs-select form-control">
									<option value="Y" ${detail.useYn == 'Y' ? 'selected' : '' }>사용</option>
									<option value="N" ${detail.useYn == 'N' ? 'selected' : '' }>미사용</option>
								</select>
							</td>
						</tr>
						<tr>
