 ...
 * @since version 5.0.1
 */
function EFormNecessaryCheckMessage(arrMessage, callBack, lng){
	...
	var displayText = lng ? lng.necessaryInfoMsg : "필수 항목이 모두 체크되지 않았습니다.";
	ClipAlertUI(displayText, message.join(""), callBack, 1, lng);
}
 ...
 */
function ClipAlertUI(title, text, callBack, type, lng){
	...
	if(type > 0){
		...
		okButton.innerHTML = lng ? lng.ok: "확인";
	...
	if(type > 1){
		...
		cancelButton.innerHTML = lng ? lng.cancel : "취소";
...
/**
 *  소켓 서버에서 데이터를 받아서 처리하기 전에 호출하는 함수입니다.<br>
 *  소켓 서버에서 받은 데이터가 strData 값으로 들어 옵니다. <br>
 *
 * @param strData {String} 소켓 서버에서 받은 데이터
 * @returns
 * @since version 1.0.0.72
 * @since version 5.0.1
 */
function onBeforeSocketReceive(strData){
	//strData = mCR_Base64.decode(strData);
	//ReportWebLog("ReceiveData:" + strData);
	return strData;
}

/**
 *  소켓 서버로 데이터를 전송이 일어나기전에 호출하는 함수입니다.<br>
 *  소켓 서버로 json text 통신이 일어나기전에 아래 선언된 함수를 거쳐서 통신합니다.<br>
 *
 * @param strData {String} 소켓 서버로 전송할 데이터
 * @returns
 * @since version 1.0.0.518
 * @since version 5.0.282
 */
function onBeforeSocketSend(strData){
	//strData = mCR_Base64.encode(strData);
