...
<script>
	...
	const eventHandler = {
		handlers: {
			...
			,createCcmButton: { handler: 'clickCreateCcmButton', eventType: 'click' }
		...
		, clickResetButton: function(e){
			const paramList = ['cdGroupId', 'cdGroupNm', 'useYn'];
		...
		, clickCreateCcmButton: function(e){
			sendForm('/adm/cdm/ccc/cmnCdClsfCreate.do', params);
		}
	...
	const setSavedParams = () => {
		...
		const idList = ['cdGroupId', 'cdGroupNm', 'useYn'];
	...
	const search = (pageNo) => {
		...
		const idList = ['cdGroupId', 'cdGroupNm', 'useYn', 'listCount'];
		for(const id of idList){
	...
	const renderTable = (list) => {
		const ccmList = document.getElementById('ccmList');
		ccmList.innerHTML = '';
		
		if(!list){
			ccmList.innerHTML = '<tr><td colspan="6">조회된 데이터가 없습니다.</td></tr>';
			return;
		}
		
		columns = ['rowNum', 'cdGroupId', 'cdGroupNm', 'useYn', 'frstRegDt', 'lastMdfcnDt']
		...
		list.forEach((data) => {
			...
			tr.addEventListener('click', (e) => {
				params['detailCdGroupId'] = data.cdGroupId;
	...
	<!-- 페이지 타이틀 시작 -->
	<h1 class="page-title-1depth"><span>공통코드 분류 목록조회</span></h1>
	<!-- 페이지 타이틀 끝 -->
	...
	<div class="content-wrapper">
		...
		<div class="contents-row">
			...
			<div class="table-bottom-control">
				<button type="button" class="btn btn-primary" id="createCcmButton">신규등록</button>
			</div>
