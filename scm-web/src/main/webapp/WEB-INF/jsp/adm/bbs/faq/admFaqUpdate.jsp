...
<script>
	...
	function fnSaveFaq() {
		...
		if(fnValid(qstnTtl, ansCn)){
			...
			sendJson('/adm/bbs/faq/updateFaq.json', params, (data) => {
				
				if(data.result == 'success'){
					return sendForm('/adm/bbs/faq/admFaqList.do');
				}
				else{
					alert('수정실패하였습니다.');
				}
	...
	<!-- 페이지 타이틀 시작 -->
	<h1 class="page-title-1depth"><span>FAQ 관리 수정</span></h1>
	<!-- 페이지 타이틀 끝 -->
				...
				<tbody>
					<tr>
						...
						<td>
							...
							<c:forEach items="${faqCategory }" var="cate">
								<option value="${cate.bbsClsfCd }" ${detail.bbsClsfCd eq cate.bbsClsfCd ? 'selected' : ''}>${cate.bbsClsfCd }</option>
							</c:forEach>
							</select>
					...
					<tr>
						...
						<td>
							...
							<input type="text" id="qstnTtl" class="form-control required" placeholder="" style="width:100%;" value="${detail.qstnTtl }">
						</td>
					</tr>
					<tr>
						...
						<td>
							...
							<textarea id="ansCn" class="form-control required" style="height: 150px;">${detail.ansCn }</textarea>
						</td>
					</tr>
				</tbody>
			</table>
